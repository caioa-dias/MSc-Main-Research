...
# --- Effective angle of attack data path ---
...
def load_and_split_data(filepath: str, AoA_effective: pd.DataFrame):
    ...
    data_val = data[data['flow_case'].isin(val_cases)]
...
def preprocess_data(data_train: pd.DataFrame, data_val: pd.DataFrame, data_test: pd.DataFrame):
    ...
    # --- Splitting the raw features into training, validation and test sets ---
    X_train_raw = data_train[X_cols].to_numpy()
    X_val_raw = data_val[X_cols].to_numpy()
    X_test_raw = data_test[X_cols].to_numpy()

    # --- Splitting the effective angle of attack into training, validation and test sets ---
    ...
    # --- Splitting the raw targets into training, validation and test sets ---
    ...
    # --- Appends the AoA feature data to the target training, validation and test data ---
    y_train = np.hstack((y_train, AoA_train_raw))
    y_val = np.hstack((y_val, AoA_val_raw))
    y_test = np.hstack((y_test, AoA_test_raw))

    # --- Saving the scalers ---
    joblib.dump(X_scaler, 'PINN-MLP/scalers/mlp_pinn_X_scaler.pkl')
    joblib.dump(y_scaler, 'PINN-MLP/scalers/mlp_pinn_y_scaler.pkl')
...
def build_model(input_dim: int, output_dim: int, panel_dx: np.ndarray, panel_dy: np.ndarray, 
    ...
    # ---Defining the physical loss weight ---
    lambda_physical = 0.094743
    ...
    x = keras.layers.Dense(units=256, activation='gelu', kernel_initializer='glorot_normal', 
    ...
    outputs = keras.layers.Dense(units=output_dim, activation='linear', kernel_initializer='glorot_normal', 
        name='Output_Layer')(x)
    ...
    pinn_supervised_mae.__name___ = 'supervised_mae'
...
def predict_test(model: keras.Model, X_test: np.ndarray, y_test: np.ndarray, 
    ...
    y_test_raw = y_scaler.inverse_transform(y_test[:, :-1])
...
def plot_data_split_parameter_space(X_train_raw: np.ndarray, X_val_raw: np.ndarray, 
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        plt.savefig(f'PINN-MLP/plots/data_split_parameter_space_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/data_split_parameter_space_{layout}.png\n')
...
def plot_training_history(history: dict, metric: str, layout: str):
    ...
    # --- Load data for loss metric plot ---
    ...
    # --- Load data for supervised MSE metric plot ---
    supervised_mse = history.history['supervised_mse']
    val_supervised_mse = history.history['val_supervised_mse']
    best_val_supervised_mse = val_supervised_mse[best_epoch_loss - 1]

    # --- Load data for physical MSE metric plot ---
    physical_mse = history.history['physical_mse']
    val_physical_mse = history.history['val_physical_mse']
    best_val_physical_mse = val_physical_mse[best_epoch_loss - 1]
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        if metric == 'loss':
            ...
            ax.set_ylabel('Loss', fontsize=label_fs, fontname='Times New Roman', 
                labelpad=lp)

        elif metric == 'supervised_mse':
            # --- Plot ---            
            plt.plot(epochs, supervised_mse, label='Training Supervised MSE', color="#303134", linewidth=main_lw)
            plt.plot(epochs, val_supervised_mse, label='Validation Supervised MSE', color='#B22222', linewidth=main_lw, 
                linestyle='--')
            plt.axvline(x=best_epoch_loss, color='#B22222', linestyle='--', alpha=0.4, linewidth=sec_lw)
            plt.scatter(best_epoch_loss, best_val_supervised_mse, label=f'Best loss epoch ({best_epoch_loss})',
                alpha=1.0, s=scatter, facecolors='#B22222', edgecolors='#000000', linewidth=0.3, 
                zorder=5)
        ...
        elif metric == 'physical_mse':
            # --- Plot ---            
            plt.plot(epochs, physical_mse, label='Training Physical MSE', color="#303134", linewidth=main_lw)
            plt.plot(epochs, val_physical_mse, label='Validation Physical MSE', color='#B22222', linewidth=main_lw, 
                linestyle='--')
            plt.axvline(x=best_epoch_loss, color='#B22222', linestyle='--', alpha=0.4, linewidth=sec_lw)
            plt.scatter(best_epoch_loss, best_val_physical_mse, label=f'Best loss epoch ({best_epoch_loss})',
                alpha=1.0, s=scatter, facecolors='#B22222', edgecolors='#000000', linewidth=0.3, 
                zorder=5)
        ...
        plt.savefig(f'PINN-MLP/plots/train_{metric}_history_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/train_{metric}_history_{layout}.png\n')
...
def plot_test_predictions(y_test_raw: np.ndarray, y_pred_raw: np.ndarray, target: str, layout: str):
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        # --- Save plot ---
        plt.tight_layout(pad=0.6)
        plt.savefig(f'PINN-MLP/plots/test_predictions_{target}_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/test_predictions_{target}_{layout}.png\n')
...
def plot_test_error_envelope_by_target(y_test_raw: np.ndarray, y_pred_raw: np.ndarray, target: str, 
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        # --- Save plot ---
        plt.tight_layout(pad=0.6)
        plt.savefig(f'PINN-MLP/plots/test_error_envelope_{target}_by_{target}_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/test_error_envelope_{target}_by_{target}_{layout}.png\n')
...
def plot_test_target_error_envelope_by_input(X_test_raw: np.ndarray, y_test_raw: np.ndarray, 
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        # --- Save plot ---
        plt.tight_layout(pad=0.6)
        plt.savefig(f'PINN-MLP/plots/test_{target}_error_envelope_by_{input}_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/test_{target}_error_envelope_by_{input}_{layout}.png\n')
...
def plot_test_cp_error_envelope_by_chord(y_test_raw: np.ndarray, y_pred_raw: np.ndarray, surface: str, 
    ...
    cp_mapping_path = 'utils/LowFidelity-CpMapping.csv'
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        # --- Save plot ---
        plt.tight_layout(pad=0.6)
        plt.savefig(f'PINN-MLP/plots/test_cp_error_envelope_{surface}_side_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/test_cp_error_envelope_{surface}_side_{layout}.png\n')
...
def plot_test_error_distribution_by_target(y_test_raw: np.ndarray, y_pred_raw: np.ndarray, target: str, 
    ...
    with plt.rc_context({'font.family': 'serif', 'font.serif': ['Times New Roman'],
        ...
        # --- Save plot ---
        plt.tight_layout(pad=0.6)
        plt.savefig(f'PINN-MLP/plots/test_{target}_error_distribution_{layout}.png', dpi=600)
        plt.close()
        print(f'Saved: PINN-MLP/plots/test_{target}_error_distribution_{layout}.png\n')
...
Final Supervised MSE (Training): {fit_losses[1]:.6f}
Final Supervised MAE (Training): {fit_losses[2]:.6f}
Final Physical MSE (Training): {fit_losses[3]:.6f}

Final Loss (Validation): {fit_losses[4]:.6f}
Final Supervised MSE (Validation): {fit_losses[5]:.6f}
Final Supervised MAE (Validation): {fit_losses[6]:.6f}
Final Physical MSE (Validation): {fit_losses[7]:.6f}
...
"""
    # Saving the report:
    with open('PINN-MLP/train-results/performance-report-MLP-PINN.txt', "w", 
        encoding="utf-8") as f: f.write(content)
    print(f"Saved: PINN-MLP/train-results/performance-report-MLP-PINN.txt\n")

    return

def main(data_path: str, airfoil_data_path: str, aoa_effective_data_path: str,):
    """
    """
    ...
    # --- Loads the effective angle of attack dataset ---
    AoA_effective = pd.read_csv(aoa_effective_data_path, sep=';')

    # --- Loads and split the dataset ---
    print('\nStarting the Multi-Layer Perceptron PINN training process...\n')
    print(f'Loading data from: {data_path}\n')
    data_train, data_val, data_test = load_and_split_data(data_path, AoA_effective)
    ...
    model = build_model(input_dim=X_train.shape[1], output_dim=y_train.shape[1]-1, panel_dx=panel_dx, 
        panel_dy=panel_dy, y_scaler=y_scaler)
    ...
    callbacks = [keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, min_delta=1e-6,
        ...
        keras.callbacks.ModelCheckpoint('PINN-MLP/train-results/MLP-PINN.keras',
            monitor='val_loss', save_best_only=True)]
    history = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=500, callbacks=callbacks, 
        batch_size=32, verbose=0)
    print('Saved: PINN-MLP/train-results/MLP-PINN.keras\n')

    # --- Evaluates the model ---
    print('Evaluationf the model...\n')
    train_loss, train_supervised_mse, train_supervised_mae, train_physical_mse = model.evaluate(X_train, 
        y_train, verbose=0)
    val_loss, val_supervised_mse, val_supervised_mae, val_physical_mse = model.evaluate(X_val, y_val, 
        verbose=0)
    fit_losses = [train_loss, train_supervised_mse, train_supervised_mae, train_physical_mse,
        val_loss, val_supervised_mse, val_supervised_mae, val_physical_mse]
    ...
    plot_training_history(history, metric='loss', layout='single_column')
    plot_training_history(history, metric='supervised_mse', layout='single_column')
    plot_training_history(history, metric='physical_mse', layout='single_column')
    ...
    # --- Saves the new raw test dataset ---
    data_test.to_csv('PINN-MLP/raw_test_dataset.csv', sep=',', index=False)
...
if __name__ == '__main__':
    main(data_path, airfoil_data_path, aoa_effective_data_path)