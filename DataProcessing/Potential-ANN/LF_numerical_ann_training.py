...
def preprocess_data(data_train: pd.DataFrame, data_test: pd.DataFrame):
    ...
    joblib.dump(scaler_x, 'LF_numerical_ann/scalers/scaler_x.pkl')
    joblib.dump(cl_scaler, 'LF_numerical_ann/scalers/scaler_cl.pkl')
    joblib.dump(cp_scaler, 'LF_numerical_ann/scalers/scaler_cp.pkl')
    
    return X_train_scaled, Y_train_scaled, X_test_scaled, Y_test_scaled, cl_scaler, cp_scaler

def build_model(input_shape: Tuple[int], output_shape: int):
    """
    ...
    model = Sequential([
        ...
        Dense(160, activation='swish', name='Hidden_Layer_1'),
        #BatchNormalization(), 
        Dropout(0.1, name='Dropout_1'),
        
        Dense(128, activation='swish', name='Hidden_Layer_2'),
        #BatchNormalization(),
        Dropout(0.1, name='Dropout_2'),
        
        Dense(224, activation='swish', name='Hidden_Layer_3'),
        #BatchNormalization(),
        Dropout(0.1, name='Dropout_3'),

        Dense(192, activation='swish', name='Hidden_Layer_4'),
        #BatchNormalization(),
        Dropout(0.1, name='Dropout_4'),

        Dense(256, activation='swish', name='Hidden_Layer_5'),
        #BatchNormalization(),
    ...
    model.compile(loss='mean_squared_error', optimizer=Nadam(learning_rate=0.001), metrics=['mae'])
...
def save_results(model: Sequential, history: History, X_test: np.ndarray, Y_test: np.ndarray, Y_pred: np.ndarray, 
    ...
    # Saving the plot:
    plt.tight_layout()
    plt.savefig('LF_numerical_ann/plots-training/training_history_loss.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/training_history_loss.png\n")
    ...
    # Saving the plot:
    plt.tight_layout()
    plt.savefig('LF_numerical_ann/plots-training/training_history_mae.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/training_history_mae.png\n")
    ...
    #plt.suptitle("Model Predictions vs. Real Values (Overall Model)", fontsize=14, fontname='Times New Roman', 
        #fontweight='bold')
    plt.xlabel("True Values", fontsize=25, fontname='Times New Roman', labelpad=20)
    plt.ylabel("Predicted Values", fontsize=25, fontname='Times New Roman', labelpad=20)
    ...
    # Saving the plot:
    plt.tight_layout()
    plt.savefig('LF_numerical_ann/plots-training/predictions_overall.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/predictions_overall.png\n")
    ...
    # Saving the plot:
    plt.tight_layout()
    plt.savefig('LF_numerical_ann/plots-training/predictions_cl.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/predictions_cl.png\n")
    ...
    # Saving the plot:
    plt.tight_layout()
    plt.savefig('LF_numerical_ann/plots-training/predictions_cp.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/predictions_cp.png\n")
    ...
    ax.set_xticks(ax.get_xticks())
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right', rotation_mode='anchor', fontsize=18)
    ax.set_yticklabels(ax.get_yticklabels(), fontsize=18)
    #plt.suptitle("Model Sensitivity: Prediction Error across Sectional Lift Coefficient Ranges", fontsize=18, 
        #fontname='Times New Roman', fontweight='bold')
    plt.xlabel("Sectional Lift Coefficient Value Range", fontsize=25, fontname='Times New Roman', labelpad=20)
    plt.ylabel("Mean Absolute Error (MAE)", fontsize=25, fontname='Times New Roman', labelpad=20)
    ...
    plt.tight_layout()
    ax.set_ylim(0, 0.03)
    plt.savefig('LF_numerical_ann/plots-training/model_sensitivity_cl.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/model_sensitivity_cl.png\n")
    ...
    #plt.suptitle("Model Sensitivity: Prediction Error across Pressure Coefficient Ranges", fontsize=14, 
        #fontname='Times New Roman', fontweight='bold')
    plt.xlabel("Pressure Coefficient Value Range", fontsize=25, fontname='Times New Roman', labelpad=20)
    plt.ylabel("Mean Absolute Error (MAE)", fontsize=25, fontname='Times New Roman', labelpad=20)
    ax.set_axisbelow(True)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    ax.set_ylim(0, 0.03)
    ...
    # Saving the plot:
    plt.tight_layout()
    plt.savefig('LF_numerical_ann/plots-training/model_sensitivity_cp.png', dpi=300)
    plt.close()
    print(f"Plot saved as LF_numerical_ann/plots-training/model_sensitivity_cp.png\n")
...
"""
    ...
    with open('LF_numerical_ann/reports/performance_report_LF_numerical_ann.txt', "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Report saved as {'LF_numerical_ann/reports/performance_report_LF_numerical_ann.txt'}\n")
...
def main(data_path: str):
    ...
    X_train, Y_train, X_test, Y_test, cl_scaler, cp_scaler = preprocess_data(data_train, data_test)
