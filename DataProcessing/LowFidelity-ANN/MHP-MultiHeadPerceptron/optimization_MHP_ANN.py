...
N_TRIALS = 41
# 27 completed - 12h 46min 18s
# 24 completed - 9h 51min 24s
# 10 completed - 3h 17min 35s
# 25 completed - 10h 33min 28s
# 20 completed - 7h 36min 51s
# 32 completed - 11h 19min 41s
# 21 completed = 8h 34min 33s
...
def objective(trial: optuna.Trial, data_train: pd.DataFrame, airfoil_data: pd.DataFrame, 
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(data_train, 
        ...
        mse_cl = skl.metrics.mean_squared_error(Cl_val_raw, Cl_pred_raw)
        mse_cp = skl.metrics.mean_squared_error(Cp_val_raw, Cp_pred_raw)
        mse_output = cl_loss_weight * mse_cl + cp_loss_weight * mse_cp
